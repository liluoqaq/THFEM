...
from dataset.frames_dataset_transformer25 import DatasetRepeater
import numpy as np
from Utils.vis_result import *
...
def train_batch_deepprompt_eam3d_sidetuning(config, generator, discriminator, kp_detector, audio2kptransformer, emotionprompt, sidetuning, checkpoint, log_dir, dataset, device_ids):
    ...
    log_dir_new = os.path.join(log_dir, 'logs')
    os.makedirs(log_dir, exist_ok=True)
    logger_new = SummaryWriter(log_dir=log_dir_new)



    with Logger(log_dir=ckpt_path, visualizer_params=config['visualizer_params'], checkpoint_freq=train_params['checkpoint_freq']) as logger:
        for epoch in trange(start_epoch, train_params['num_epochs']):
            ...
            for x in tqdm(dataloader):
                ...
                if count == 0 or count%1000 == 0:
                    # save_path = os.path.join(log_dir,"vis_output")
                    # if count == 0:
                    #     os.makedirs(save_path,exist_ok=True)
                    vis_result(x["source"],x["driving"][0],generated["prediction"],count,log_dir)
                    log_metrics(logger_new,losses,"train",count)
                count += 1
                if count % 100 == 0 :
                    # print(np.array(logger.loss_list).mean(axis=0), count%len(dataloader), len(dataloader))
                    print_metrics(losses,count,prefix="train")
                logger.log_iter(losses=losses)
