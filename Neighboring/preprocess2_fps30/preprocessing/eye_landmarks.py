...
def detect_landmarks(img_paths, predictor, device, template=None, multisample=True):
    landmarks = []
    prev_points = None
    align_transforms = []
    prev_aligns = None
        ...
        if preds is not None:
            ...
            if template is not None:
                if multisample:
                    ...
                    for x in tx:
                        for y in ty:
                            mat = np.array([[1, 0, x], [0, 1, y]]).astype(float)
...
def main():
    ...
    template = np.loadtxt(
        'preprocess2/preprocessing/files/template.gz') if args.align else None  # template 68 landmarks (for face alignment)

    # Get the path of each image.
    images_dir = os.path.join(args.celeb, 'images')
    image_paths = get_image_paths(images_dir)

    if True:  #not dirs_exist(image_paths):
        landmarks, align_transforms = detect_landmarks(image_paths, predictor, device, template=template)
