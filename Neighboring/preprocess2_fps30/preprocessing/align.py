...
def save_aligned(mat_pths, aligned, args):
    ...
    out_paths = []
    if args.images:
        out_paths += [p.replace('/align_transforms/', '/images_aligned/') for p in mat_pths]
    ...
    for al, mat_pth in tqdm(zip(aligned, mat_pths), total=len(mat_pths)):
        ...
        if args.images:
            img_a = next(al_iter)
            img_file = os.path.splitext(mat_pth.replace('/align_transforms/', '/images_aligned/'))[0] + '.png'
            cv2.imwrite(img_file, img_a)
        if args.faces_and_masks:
...
def align(mat_paths, args):
    ...
    for mat_pth in tqdm(mat_paths):
        ...
        img_pth = os.path.splitext(mat_pth.replace('/align_transforms/', '/images/'))[0] + '.png'
        if not os.path.exists(img_pth):
            continue
        if args.images:
            img = cv2.imread(img_pth)
            img_a = cv2.warpAffine(img, mat, (img.shape[1], img.shape[0]), flags=cv2.INTER_LANCZOS4)
            ret.append(img_a)
...
def main():
    ...
    parser.add_argument('--images', action='store_true', help='Whether to images')
...
if __name__ == '__main__':
    main()
#python preprocessing/align.py  --celeb  /data/2023_stu/zhenxuan/codespace/EAT_code-main/preprocess_1  --images --landmarks
#python preprocessing/eye_landmarks.py  --celeb /data/2023_stu/zhenxuan/codespace/EAT_code-main/preprocess_1 --align