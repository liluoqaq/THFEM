...
def draw_annotation_box(image, rotation_vector, translation_vector, color=(255, 255, 255), line_width=2):
    ...
    camera_matrix = np.array([[233.333, 0, 128], [0, 233.333, 128], [0, 0, 1]], dtype="double")
...
def main(args):
    all_latents = glob.glob(f'{args.root}/latents/*.npy')
    all_latents.sort()
    os.makedirs(f'{args.root}/poseimg', exist_ok=True)
    for lat in tqdm(all_latents):
        out = basename(lat)[:-4]
        outpath = f'{args.root}/poseimg/{out}.npy.gz'
        print(outpath)
        if exists(outpath):
            try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", default="*", help="filenames")
