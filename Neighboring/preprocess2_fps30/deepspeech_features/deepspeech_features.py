...
def conv_audios_to_deepspeech(audios,
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        # print(len(audios))
        # print(len(audios))
        for audio_file_path, out_file_path, num_frames in tqdm(zip(audios, out_files, num_frames_info)):
...
def pure_conv_audio_to_deepspeech(audio,
    ...
    deepspeech_fps = 60
    video_fps = 60  # Change this option if video fps is different
...
#     print(network_output.shape) # 368, 29
#     assert(0)
    # Make windows:
