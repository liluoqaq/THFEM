...
def keypoint_transformation(kp_canonical, he, estimate_jacobian=True, dkc=None):
    ...
    rot_mat = get_rotation_matrix(yaw, pitch, roll).cuda()    # (bs, 3, 3)
    ...
    t = t.unsqueeze_(1).repeat(1, kp.shape[1], 1).cuda()
    kp_t = kp_rotated + t

    # add expression deviation 
    exp = exp.view(exp.shape[0], -1, 3).cuda()
...
class GeneratorFullModelBatch(torch.nn.Module):
    """
    Merge all generator related updates into single model for better multi-gpu usage
    """

    def __init__(self, kp_extractor, audio2kptransformer, generator, discriminator, train_params, estimate_jacobian=True):
        super(GeneratorFullModelBatch, self).__init__()
        ...
        if self.loss_weights['sync'] != 0:
            ...
            syncnet_state = torch.load('./ckpt/checkpoint_epoch02600_testloss0.8152778898516009_testacc0.5245564512668117.pth', map_location='cpu')
        ...
        if self.loss_weights['clip'] != 0:
            self.clip_model, self.clip_preprocess = clip.load("./ViT-B-32.pt")
            self.text_token = self.get_emotoken()
            if torch.cuda.is_available():
    ...
    def forward(self, x, train_with_img=False):
        ...
        ### a2kp
        he_driving_emo = self.audio2kptransformer(x, kp_canonical)           # {'yaw': yaw, 'pitch': pitch, 'roll': roll, 't': t, 'exp': exp}
        ...
        he_new_driving = {'yaw': he_driving['yaw'], 'pitch':he_driving['pitch'], 'roll':he_driving['roll'], 't':he_driving['t'], 'exp': exp}  #新的表情关键点参数
        ...
        if train_with_img:
            ...
            # with torch.no_grad():
            generated = self.generator(source, kp_source=kp_source, kp_driving=kp_driving_bbs)
            ...
            if self.loss_weights['clip'] != 0:
                ...
                similarity = 1 - self.clip_model(processed_img, self.text_token.to(device))[0][:, y_trg] / 100
                # print(similarity[0])
            ...
            if self.loss_weights['sync'] != 0:
                ...
                if 'img_l1' in self.loss_weights.keys():
                    loss_values['img_l1'] = self.loss_weights['img_l1'] * torch.abs(gt_bbox-pre_bbox).mean()
                else:
                    loss_values['img_l1'] = torch.abs(gt_bbox-pre_bbox).mean()
    ...
    def get_emotoken(self):
        ts4 = ['The person is talking angrily.', 
                ...
                'The person is talking without emotion.',
                'The person is talking sadly.',
                'The person is talking surprisingly.']
        return clip.tokenize(ts4)
...
class DiscriminatorFullModel(torch.nn.Module):
    ...
    def forward(self, x, generated):
        pyramide_real = self.pyramid(x['driving'].squeeze(0))
...
class GeneratorFullModelBatchDeepPromptSTEAM3D(torch.nn.Module):
    ...
    def forward(self, x, train_with_img=False):
        ...
        with torch.no_grad():
            kp_canonical = self.kp_extractor(x['source'], with_feature=True)     # {'value': value, 'jacobian': jacobian} 图像经过关键点检测器
        kp_cano = kp_canonical['value']   #原图像的3维关键点 （batch,15,3）
        he_source = x['he_source']

        ### emotion prompt
        emoprompt, deepprompt = self.emotionprompt(x)  #(batch,128)  (batch,6,128)
        # print(emoprompt.shape)
        ### a2kp
        he_driving_emo, input_st = self.audio2kptransformer(x, kp_canonical, emoprompt=emoprompt, deepprompt=deepprompt, side=True) #得到n×32
        emo_exp = self.sidetuning(input_st, emoprompt, deepprompt)  #得到情感表达关键点
        he_driving = x['he_driving']
        exp = he_driving_emo['emo']
        device = exp.get_device()
        ...
        ### transfer pca exp to kp exp
        exp = torch.mm(exp, self.expU.t().to(device))  #对a2kp得到的关键点pca
        exp = exp + self.expmean.expand_as(exp).to(device)
        exp = exp.reshape(bbs, bs, 45) # 45: 15 point * 3 xyz
        ...
        kp_driving = []
        for i in range(bbs):
            source_area = ConvexHull(kp_cano[i].cpu().numpy()).volume  ##？？（1，15，3）的关键点，得到一个系数
            source_areas.append(source_area)
            exp[i] = exp[i] * source_area
            kp_canonical_i = {'value': kp_cano[i].unsqueeze(0)}   #source图像的关键点
            he_new_driving_i = {'yaw': he_driving['yaw'][i], 'pitch':he_driving['pitch'][i], 'roll':he_driving['roll'][i], 't':he_driving['t'][i], 'exp': exp[i]}
            kp_driving.append(keypoint_transformation(kp_canonical_i, he_new_driving_i, self.estimate_jacobian)['value'])  #生成的关键点信息，将原始关键点旋转平移➕表情参量
        ...
        if train_with_img:
            ...
            ### Note: batch keypoint transformation operation is equal to 'for' iteration in every he source
            kp_source = keypoint_transformation(kp_canonical, he_source, self.estimate_jacobian)  #原图的关键点信息
            bbs = kp_canonical['value'].shape[0]
            bs = kp_driving[0].shape[0]
            rep = torch.LongTensor([bs]*bbs).to(device)
            kp_source['value'] = torch.repeat_interleave(kp_source['value'], rep, dim=0)
